import type { CompList, standardBlock } from '../types/types'
...
> {
  ...
  useTemplate() {
    const slotTemplate = Object.entries(
  ...
  useScript() {
    const slotScript = Object.entries(
      this.slotCode as { [key in string]: Content[] },
...
export class sourceGenerator<
  BlockType extends standardBlock<BlockType>, NodeType extends {
    tag: string
    children: NodeType[]
    slot?: string
...
> {
  ...
  useSlot(slotSet: string[], allCompList: CompList<any>, renderType: string) {
    slotSet.forEach((templateName) => {
      // eslint-disable-next-line array-callback-return
      const childrenNodes = this.block.blocks.map((block: BlockType) => {
        try {
          if (block.slot === templateName)
            return (allCompList as any).get(block.key)[renderType](block)
        }
