...
describe("normal action on engine instance", () => {
  ...
  it("root instance in blockMap", () => {
    expect(instance.find("1").testKey).toEqual("UUID");
  });

  it("add block to root", () => {
    instance.add({ uuid: "test1", key: "test1", blocks: [] });
    expect(instance.find("test1").uuid).toEqual("test1");
  });

  it("add block to specifier", () => {
    instance.add({ uuid: "test2", key: "test2", blocks: [] }, "test1");
    expect(instance.find("test1").blocks.length).toEqual(1);
  });

  it("remove a block", () => {
    instance.remove("test1");
    expect(instance.find("1").blocks.length).toEqual(0);
    expect(instance.find("test2")).toEqual(undefined);
  });
  it("atom action ", () => {
    instance.add({
      uuid: "test3",
      key: "test3",
      width: { value: 1, size: 'px' },
      height: { value: 1, size: 'px' },
      blocks: [],
    })

    instance.keep('test3', ["width", 'height.value', 'key'])
    let block = instance.find("test3")
    block.width = { value: 10, size: 'px' }
    block.height.value = 5
    block.key = 'test4'
    instance.back()
