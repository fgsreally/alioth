  ...
  methods: {
    ...
    onExecCmd(key, command, success, failed) {
      ...
      if (key === 'fail') {
        ...
        const info = TerminalApi.elementInfo(this.name)
        success({
          type: 'json',
    ...
    async showFlash(success) {
      ...
      const terminalInfo = TerminalApi.elementInfo(this.name)
    ...
    mockLoading(flash, fileName, terminalInfo) {
      ...
      const fixedWidth = 15 + (6 + fileName.length) * terminalInfo.charWidth.en + 20
      //  计算出进度条的 '-' 个数
      ...
      return new Promise((resolve) => {
        ...
        const flashInterval = setInterval(() => {
          ...
          const str = `${prefix1 + (90 * (count % 8)) + prefix2 + '#'.repeat(count) + '-'.repeat(processDots - count)}]${percent}%`
          //  更新显示当前进度
          flash.flush(str)
    ...
    >
      <template #textEditor="{ data }">
        ...
        <div class="t-text-editor-floor" align="center">
          <button class="t-text-editor-floor-btn t-save-btn" @click="_textEditorClose(true)">
            Save & Close(Ctrl + S)
          </button>
          <button class="t-text-editor-floor-btn t-close-btn" @click="_textEditorClose(false)">
            Cancel
          </button>
        </div>
      </template>
